...
`;
  ...
  code = code.replace(
    /app\.post\("\/api\/v1\/hr\/clearance\/:clearanceId\/resolve-blocker", \(req, res\) => \{[\s\S]*?res\.json\(\{ success: true, clearance \}\);\s*\}\);/,
...
`
  ...
  // Remove local data arrays & initStore
  code = code.replace(/let clearancesList.*?;.*?function saveLocalDatabase\(\) \{.*?\}/s, '');
  code = code.replace(/let auditTrailsList: any\[\] = \[.*?\];/s, '');
  code = code.replace(/function initStore\(\).*?\}/s, '');
  code = code.replace(/initStore\(\);/s, '');
