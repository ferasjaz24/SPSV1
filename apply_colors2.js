...
files.forEach(file => {
  ...
  let oldProcStatus = /<span className=\{\`px-3 py-1 rounded-full border text-\[10px\] font-black uppercase tracking-wide \$\{[\s\S]*?req\.status === 'قيد المراجعة' \? 'bg-blue-50 text-blue-700 border-blue-100' :[\s\S]*?'bg-slate-50 text-slate-600 border-slate-200'\s*\}\`\}>/g;
  ...
  let oldAssFull = /<span className=\{\`mr-2 px-2 py-0\.5 rounded-full text-\[9px\] font-black \$\{[\s\S]*?associatedProc\.status === 'تم استلام المواد' \? 'bg-emerald-50 text-emerald-800 border-emerald-100' :[\s\S]*?'bg-slate-50 text-slate-700 border border-slate-200'\s*\}\`\}>/g;
  ...
  if (content.includes('const getStatusColor =') && content.includes('case \'في انتظار المراجعة\':')) {
